...
| ⚡ **高效能** | 智慧快取機制 | 降低重複查詢的 API 成本 |
| 🛡️ **企業級** | 完整錯誤處理 | 分層例外與重試策略 |
| 📊 **即時監控** | Prometheus + Grafana | 即時系統狀態 |
| 🚀 **效能優化** | 向量檢索效能監控 | 檢索延遲可觀測 |
| 🔍 **可觀測性** | 結構化日誌 + 分散式追蹤 | 完整請求鏈路追蹤 |
| 💾 **狀態管理** | 工作流程狀態持久化 | 支援中斷恢復 |
| 🔄 **容錯機制** | 智慧重試策略 | 提升系統可靠性 |

> **關於量化數字**：先前版本在此表列出「85% API 成本節省」「85%+ 測試覆蓋率」
> 「P95 < 200ms」等數字，但專案內並無對應的實測依據，且 CI 的覆蓋率門檻實際為
> 50%（見 `.github/workflows/ci-cd.yml` 的 `MIN_COVERAGE`）。為避免誤導，
> 已改為描述能力而非宣稱未經量測的數值。這些仍是合理的**目標值**，
> 待補上實測後再標註。
...
# 追蹤配置（一律走 OTLP；Jaeger 原生支援 OTLP 接收）
